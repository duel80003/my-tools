...
func InitializeConsumer(config *RmqConfig, consumerConfig *ConsumerConfig) {
	...
	if err != nil {
		Logger.Fatalf("NewPushConsumer error: %s", err)
	}

	err = c.Subscribe(consumerConfig.Topic, getMessageSelector(consumerConfig), consumerConfig.MsgHandler)
	if err != nil {
		Logger.Fatalf("subscribe error: %s", err)
	}

	err = c.Start()
	if err != nil {
		Logger.Fatalf("start consumer error: %s", err)
	}
...
func gerGroupName(config *ConsumerConfig) string {
	if config.Group == "" {
		Logger.Fatal("consumer group name is required")
	}
	return config.Group
}
